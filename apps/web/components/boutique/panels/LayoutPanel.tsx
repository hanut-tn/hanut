...
/** Panneau flottant fixe (comme HeaderPanel) — déclenché en cliquant dans la
 * grille produits hors d'une carte (gouttière). */
export default function LayoutPanel({ layout, onChange, onClose }: Props) {
  return (
    <div
    ...
    >
      <div className="sticky top-0 flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-white rounded-t-2xl">
        <span className="font-semibold text-sm text-gray-900">Disposition</span>
        <button
          type="button"
          onClick={onClose}
          aria-label="Fermer"
      ...
      <div className="p-4">
        <LayoutSection layout={layout} onChange={onChange} />
      </div>
    </div>
  )
}
