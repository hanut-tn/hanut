...
export default function BackgroundPanel({ pageBg, onChange, position, onClose }: Props) {
  ...
  return (
    ...
    >
      <div className="sticky top-0 flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-white rounded-t-2xl">
        <span className="font-semibold text-sm text-gray-900">Fond de la page</span>
        <button
          type="button"
          onClick={onClose}
          aria-label="Fermer"
      ...
      <div className="p-4">
        <ColorField label="Couleur" value={pageBg} presets={COLOR_PRESETS.pageBg} onChange={onChange} />
      </div>
    </div>
  )
}
