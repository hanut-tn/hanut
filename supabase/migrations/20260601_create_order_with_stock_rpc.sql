...
DO $$
BEGIN
  IF to_regprocedure('decrement_stock(uuid, integer)') IS NOT NULL THEN
    EXECUTE 'ALTER FUNCTION decrement_stock(UUID, INTEGER) SECURITY INVOKER';
    EXECUTE 'REVOKE ALL ON FUNCTION decrement_stock(UUID, INTEGER) FROM PUBLIC';
    EXECUTE 'GRANT EXECUTE ON FUNCTION decrement_stock(UUID, INTEGER) TO authenticated, service_role';
  END IF;
END $$;
