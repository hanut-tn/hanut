-- Correction performance : get_analytics_data — sous-requêtes corrélées → LATERAL JOIN
-- Le calcul total_cost utilisait une double sous-requête corrélée (N × M requêtes).
-- Remplacé par LATERAL JOIN sur order_items pré-agrégé par order_id.
