...
-- Supprimer l'ancienne signature 13 paramètres avant de recréer en 14.
...
BEGIN
  ...
  INSERT INTO orders (
    ...
    quantity, cod_amount, unit_cost, notes, status, tracking_token, customer_email
  )
  VALUES (
    p_seller_id, v_customer_id, p_product_id, v_variant,
    p_quantity, v_cod_amount, COALESCE(v_product.cost, 0), v_notes, p_status,
    replace(gen_random_uuid()::text, '-', ''), v_customer_email
  )
