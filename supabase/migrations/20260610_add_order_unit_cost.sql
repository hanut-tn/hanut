...
-- Étape 1 : Ajouter la colonne unit_cost sur orders.
ALTER TABLE orders
  ADD COLUMN IF NOT EXISTS unit_cost NUMERIC;

-- Étape 2 : Rétroactivement remplir unit_cost pour les commandes existantes
-- en lisant products.cost au moment de la migration.
-- Les commandes futures seront renseignées par la RPC ci-dessous.
UPDATE orders o
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid = 'orders'::regclass
      AND conname = 'orders_unit_cost_nonnegative'
  ) THEN
    ALTER TABLE orders
      ADD CONSTRAINT orders_unit_cost_nonnegative CHECK (unit_cost >= 0);
  END IF;
END $$;

-- Étape 3 : Recréer create_order_with_stock pour inclure unit_cost dans l'INSERT.
-- Même signature 13 paramètres que 20260609_stock_variant_consistency.sql —
-- CREATE OR REPLACE conserve les permissions existantes.
